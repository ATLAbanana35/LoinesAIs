...
f = open("model_trained.json", "r")
sterilized = json.load(f)
f.close()
W = np.array(sterilized[0])
b = np.array(sterilized[1])
print(W.shape)
class bcolors:
    HEADER = '\033[95m'
    OKBLUE = '\033[94m'
...
class LaiModel:
    ...
    def execute(self, XX):
        ...
        return self._A_ >= 0.5
    def predict_single_image(self, image_path, out):
        ...
        if prediction:
            out.write(("<h1>C'est un téléphone! </h1>").encode())
        else:
            out.write(("<h1>C'est une maison! </h1>").encode())
        return flattened_image.reshape(1, -1)

# Create an instance of the model
LaiINSTANCE = LaiModel()
LaiINSTANCE.b = b
LaiINSTANCE.W = W
html_code = '''
<!DOCTYPE html>
...
<head>
    ...
    <title>Simple HTTP Server with Upload</title>
</head>
<body>
    <h2>Upload a File</h2>
...
class SimpleHTTPRequestHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        r, info, fI = self.deal_post_data()
        print((r, info, "by: ", self.client_address))
        f = BytesIO()
        LaiINSTANCE.predict_single_image(fI, f)
        length = f.tell()
        f.seek(0)
